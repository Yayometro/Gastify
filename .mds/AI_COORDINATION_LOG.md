...
### 📅 Entry #28: 2026-08-24/25 (local time) — Real non-MXN account test (found+fixed 8 currency bugs), Phase 11 backup + scoped 10-transaction migration

- **AI Assistant**: Claude (Sonnet 5)
- **User Request**: Explained Phase 9/11 in detail on request before deciding anything. User asked to confirm the FX mechanism for Phase 9 (historical per-transaction ECB rate vs "current" rate - confirmed: closed/past transactions use their own historical snapshot, only present/future items like starting balance or future income projections use the latest available rate, explicitly marked estimated). Then, independent of Phase 9/11, asked to create a real "Revolut" USD account plus a $3,000 test transaction (as an income, per a follow-up correction) to see how the whole system behaves with a genuinely non-MXN account for the first time. Then approved the Phase 11 plan (local backup, then a small 10-transaction real test) and asked whether the backup should live locally or inside MongoDB itself (answered: locally - a backup inside the same database/cluster shares its failure domain, doesn't protect against anything).
- **Phase**: Ad-hoc real-account verification (found real bugs, not part of a plan phase); Phase 11 first real steps (backup + scoped test)
- **Actions Taken**:

  1. **Built `scripts/backup_multicurrency_migration.js`** - read-only, dumps full documents from all 7 collections the migration touches into timestamped local JSON files under `backups/` (gitignored - real financial data never committed). First real run: 3,143 transactions / 17 accounts / 12 wallets / 30 budgets / 2 incomeSources / 2 projectionSettings / 59 categoryRules, matching the existing dry-run audit exactly.

  2. **Created real test data with explicit user permission**: Account "Revolut" (USD, via the real Create Account UI) and Transaction "Test income - Revolut USD" ($3,000, isIncome, via the real Add Transaction UI, using the Charged-Elsewhere/live-FX-preview flow from Phase 5). Confirmed via direct Mongo read that money.account = {amountMinor: 300000, currency: "USD"} and money.reporting = {amountMinor: 5069408, currency: "MXN", rate: "16.898...", source: "ecb_reference", estimated: true} - a real, correct ECB conversion.

  3. **Found and fixed 8 real currency-mislabeling bugs**, all the same root cause as the Top3.jsx bug from Task 8.3 but never previously exposed because every Account in this database predated the currency migration and defaulted to MXN - summing raw `transaction.amount` (native currency) and displaying it labeled as the Wallet's primary currency is only correct by coincidence when everything happens to already be MXN. The $3,000 USD transaction rendered as a bare mislabeled "MXN $3,000.00" (should be "MXN $50,694.08" or native "USD $3,000.00") in:
     - `CreditCard.jsx` (per-account income/bill/total reduce, on the Accounts page)
     - `Dashboard.jsx` (the wallet-header Incomes/Expenses totals)
     - `src/helpers/timeFunctions/timeFunctions.js` - `orderItemsInTheirMonth` and `slicedAndReduceNewValuesForMonths`, both summing `item.value || item.amount` over raw transactions that never had `.value` set. This is a **different file** from `transactionsChange.js` (which Phase 8.2 already fixed) and was never touched by any prior phase - the actual root of the History/Dashboard "Top 6 Transactions" (not Categories) pipeline. `getPrimaryAmount` is duplicated locally here rather than imported, since `transactionsChange.js` itself imports from `timeFunctions.js` and importing back would be circular.
     - `TopMonthItem.jsx`, `TopItemContainer.jsx`, `TopContainer.jsx`, `TopMonthContainer.jsx`, `ModalContentTopMonthItem.jsx` - display components reading `item.value || item.amount` directly on raw transaction objects (assuming the field name meant "already-aggregated category total," which is true for the Top-6-*Categories* path but not the Top-6-*Transactions* path they're also reused for). Replaced with `getPrimaryAmount(item)`, a safe superset: converts a real transaction via `displayMoney.primary`, or falls back to `.value` exactly as before for an already-aggregated synthetic category object.
     - Verified end to end after each fix: Accounts, Dashboard, Movements, and every History Top-6/modal view (both Bills/Incomes toggle and both Transactions/Categories tabs) now show the Revolut transaction correctly as native "USD $3,000.00" or converted "MXN $50,694.08", never a bare mislabeled number. 145/145 tests, zero new lint errors throughout.
     - Real test data left in the database on purpose (unlike the Phase 10 upload test, which was cleaned up) - the user wants to keep using it to watch how the system behaves with a genuine non-MXN account going forward.

  4. **Phase 11 - added `--only=<names>` and `--limit=<n>` flags to `scripts/migrate_multicurrency.js`**, so a small subset can be trialed before the full unbounded run. Ran a real, explicitly-approved scoped write: `--only=transactions --limit=10 --confirm`. Read the exact 10 target documents first (all real 2024 historical data, nothing synthetic) and showed them to the user before writing. Verified after the write via direct Mongo read: all 10 correctly got `kind`/`direction`/`state`/`money` (source `legacy_migration`, rate `1`, MXN), with the legacy `amount` field completely untouched, matching plan section 21.4 exactly.
     - **Found in passing, not a bug in this migration**: 9 of the 10 migrated documents (all created in one bulk-import batch on 2024-03-01, per their shared `createdAt`) have a corrupted `date` field pinned near the Unix epoch (`1970-01-01T00:00:45.3xxZ`) instead of their real transaction date - `createdAt` is fine, only the user-facing `date` field is wrong. The migration script faithfully carried this pre-existing bad value into the new `money.reporting.effectiveDate`, exactly as it should (it's not the migration's job to fix unrelated data quality issues). Side effect noticed while trying to view these 10 transactions in the UI: none of the date-range presets (This Month/Last Month/quarters/All 2025/All 2026) can show them, since their `date` is stuck at 1970 - they're only reachable via the app today with a custom date range spanning 1970. Not fixed - flagged here for whoever eventually audits that import batch.

- **Files Created / Modified**:
  - Backup: `scripts/backup_multicurrency_migration.js` (new), `.gitignore` (added `/backups/`)
  - Currency bug fixes: `src/components/Dashboard.jsx`, `src/components/multiUsedComp/CreditCard.jsx`, `src/helpers/timeFunctions/timeFunctions.js`, `src/components/multiUsedComp/top3/topMonthContainer/{TopMonthItem,TopItemContainer,TopMonthContainer}.jsx`, `src/components/multiUsedComp/top3/top-container/TopContainer.jsx`, `src/components/modals/contents/modalForTopMonthItem/ModalContentTopMonthItem.jsx`
  - Migration script: `scripts/migrate_multicurrency.js` (added `--only`/`--limit`)
  - This entry: `.mds/AI_COORDINATION_LOG.md`
  - Database (all with explicit user permission): Account "Revolut" (USD) + Transaction "Test income - Revolut USD" ($3,000, left in place); 10 real legacy Transaction documents migrated in place (kind/direction/state/money added, nothing removed).
- **Test status at end of session**: 145/145 vitest passing, zero new `next lint` errors, on `claude/multicurrency` (not merged to `develop`/`main`).
- **Next Steps / Hand-Off Notes**:
  - Local backup exists at `backups/multicurrency-pre-migration-2026-08-24T20-53-26-442Z/` (gitignored, on this machine only - not synced anywhere else). If continuing the full unbounded `--confirm` migration later, consider taking a fresh backup first if meaningful time/data has passed since this one.
  - 10 of ~3,143 transactions are now migrated; the rest (plus all wallets/accounts/budgets/incomeSources/projectionSettings/categoryRules) are still pending the full run - `node --env-file=.env scripts/migrate_multicurrency.js --confirm` with no `--only`/`--limit` would do the rest, but per this session's discussion the user is intentionally not rushing this (real visible impact is ~zero for a single-currency wallet; it's an internal correctness/simplification investment, not a user-facing feature).
  - Phase 9 (real Projections/IncomeSources currency conversion) still not started - confirmed with the user how the historical-vs-current ECB rate split should work; also agreed to add a simple manual-override control to the transaction edit UI for correcting a bad conversion (the underlying `manualReportingAmount` mechanism already exists, just needs a more visible entry point than the two narrow places it's currently exposed - account-currency-reassignment strategy picker, and Excel Reporting Amount column).
  - Given today's findings, before/whenever Phase 9 work starts, it would be worth a systematic `rg "\.amount\b" src --include=*.js --include=*.jsx | grep reduce` sweep (or similar) across the whole codebase rather than fixing instances reactively as new real multi-currency data surfaces them - today's 8 bugs were only found because the user asked for a real non-MXN account test, not through a systematic audit.
  - Standing rule unchanged: full autonomy for code/schema/route/test changes; explicit permission required before any real MongoDB document write. Both real-data actions this session (the Revolut test account/transaction, the scoped 10-transaction migration) were explicitly approved via `AskUserQuestion` beforehand.
  - `claude/multicurrency` is not yet merged anywhere.

---

### 📅 Entry #29: 2026-08-25 (local time) — Phase 11 full migration write, Phase 9 real Projections conversion

- **AI Assistant**: Claude (Sonnet 5)
- **User Request**: Run the full unbounded `--confirm` migration, then move to Phase 9 - with an explicit instruction to explain the plan before writing any Phase 9 code (not just start). User also clarified the real "Revolut" test Account (created in Entry #28) should stay in USD permanently - they intend to keep using it as an actual account, not just disposable test data.
- **Phase**: Phase 11 (full migration write) complete; Phase 9 (Projections real conversion) complete, Budget-currency math still deferred
- **Actions Taken**:

  1. **Phase 11 - full migration**: fresh local backup first (`backups/multicurrency-pre-migration-2026-08-25T01-58-07-191Z/` - accounts grew from 17→18 since the last backup, confirming Revolut and other real activity since Entry #28). Dry-run confirmed exact expected counts (11 wallets / 17 accounts / 3,131 transactions / 30 budgets / 2 incomeSources / 2 projectionSettings / 59 categoryRules still pending - exactly 10 fewer transactions than the original full count, matching the Entry #28 scoped test). Ran `--confirm` with no `--only`/`--limit`. Post-migration audit: 100% of every collection migrated, before/after totals identical to the cent (accounts $905,607.00, transactions $6,054,091.73 - drift $0.00), re-running the dry-run script afterward returned all zeros (idempotency confirmed). Live UI spot-check (Dashboard reload) showed no visible change and no console errors, as expected.
     - Audit's `categoryRules.alreadyMigrated: 3` (not 59) turned out to be an audit-script quirk, not a real gap: its check requires `minAmountMinor !== null`, but 56 of the 59 rules never had a `minAmount` threshold to begin with (name/category-only rules) - migration correctly wrote `minAmountMinor: null` for those, which is right, just not what that one audit line's check expected. Verified directly: all 59 have `amountCurrency` set and the `minAmountMinor` key present. Not fixed (the audit script itself isn't wrong to flag it conservatively, and it doesn't affect real migrated data) - noted here for whoever next reads that audit output and wonders about the same discrepancy.
     - Also noticed while investigating a Projections math discrepancy in the follow-up Phase 9 work below: one of the two Phase 7 transfer-test legs (`_id: 6a890eed...37d2`) no longer exists in the database - the other (`...37d3`) is now an orphaned single leg. Not caused by anything in this session's traceable actions; flagged, not repaired (a full transfer needs both legs deleted/recreated together per the app's own transfer semantics, and this orphan doesn't corrupt anything now that `filterBillsOrIncomes` correctly excludes it - see below).

  2. **Phase 9 - explained the plan before writing code, per explicit user request**, then executed it once approved:
     - `src/helpers/transformers/projectionsChange.js`: `buildYearProjectionTable`'s `actualIncome`/`actualExpense` and `sumPerBucketMax`/`getMonthBucketBreakdown`'s bill sums now use `getPrimaryAmount(tx)` (imported from `transactionsChange.js` - no circularity, that file doesn't import back) instead of raw `tx.amount`. `Budget.goalAmount`/`IncomeSource.amount` deliberately left untouched (still implicit wallet-primary) - real Budget-currency conversion is section 16's larger, separately-scoped work, not attempted here.
     - `ProjectionsClient.jsx`: `startingBalance` converted from a synchronous raw `account.amount` reduce into an async effect that live-quotes each non-credit Account's native balance into the Wallet's primary currency (reusing the `general-data/fx/quote` route, same pattern as `CreditCard.jsx`'s existing equivalent-balance feature) - a quote that's unavailable is skipped, never faked, matching the project's standing "never fake a rate" rule.
     - `projections/update/route.js`: `monthlyBalances[].money` and `monthlyBuffers[].{expenseMoney,incomeMoney}` (additive fields sitting unused in the schema since Phase 3) are now populated on every save, stamped with the Wallet's current primary currency - old entries keep their MXN-implicit legacy meaning forever, matching plan section 7.6's "changing Wallet primary currency does not reinterpret old manual values."
     - `EditSingleTransModal.jsx`: new "Doesn't look right? Correct it" link next to the live FX preview on any transaction whose Account currency differs from Wallet primary - opens a pre-filled editable field wired to `manualReportingAmount` (the edit route already fully supported this, just never had a general-purpose entry point beyond the narrower Account-currency-reassignment and Excel-import paths).
     - **Found and fixed a second real bug via the same live-testing discipline**: verifying the Revolut transaction's contribution to Projections showed "$50,794.08" instead of the correct "$50,694.08" - a stray extra $100 traced to `filterBillsOrIncomes()` (shared by Dashboard/Top3/History/Projections) never excluding transfer/exchange legs. A transfer leg is `isBill: false, isIncome: false` by design, so `!tra.isBill` alone classified it as income - this bug has existed since Phase 7 shipped, invisible until a transfer leg's amount was large/present enough in a period being actively displayed. Fixed by excluding `kind === "transfer" || kind === "exchange"` before the isBill/isIncome split (safe now that every Transaction has a real `kind` field post-Phase-11-migration). Re-verified: Revolut's "real income so far" now shows exactly $50,694.08.
     - Tested the manual-override control for real: opened the Revolut transaction, entered a deliberately-wrong 51000 correction, confirmed via direct Mongo read it saved `money.reporting = {amountMinor: 5100000, rate: "17", source: "manual", estimated: false}` exactly as expected, then corrected it back to the real 50694.08 through the same control (now `source: "manual"` permanently for this one transaction, since the user explicitly confirmed a value once - this is correct/intended, not something to "undo").
  3. **New tests**: `projectionsChange.test.js` (new - `buildYearProjectionTable`/`getMonthBucketBreakdown` currency correctness, plus a legacy-no-displayMoney fallback case) and two new cases added to `transactionsChange.test.js` for `filterBillsOrIncomes`'s transfer-exclusion fix.

- **Files Created / Modified**:
  - Phase 9: `src/helpers/transformers/projectionsChange.js`, `src/helpers/transformers/projectionsChange.test.js` (new), `src/components/multiUsedComp/Projections/ProjectionsClient.jsx`, `src/app/api/general-data/projections/update/route.js`, `src/components/multiUsedComp/EditSingleTransModal.jsx`, `src/helpers/transformers/transactionsChange.js`, `src/helpers/transformers/transactionsChange.test.js`
  - This entry: `.mds/AI_COORDINATION_LOG.md`, `.mds/MULTI_CURRENCY_IMPLEMENTATION_PLAN.md` (status header)
  - Database: full `--confirm` migration write across all 7 collections (every Wallet/Account/Transaction/Budget/IncomeSource/ProjectionSettings/CategoryRule document in the database), explicitly approved beforehand. One real Transaction edited twice by this session's own manual-override testing (Revolut income, corrected back to its accurate value afterward).
- **Test status at end of session**: 150/150 vitest passing, zero new `next lint` errors, on `claude/multicurrency` (not merged to `develop`/`main`).
- **Next Steps / Hand-Off Notes**:
  - The multi-currency migration proper (Phases 0-11 as scoped, minus the deliberately-deferred Budget-currency math) is now essentially complete. What remains, if ever needed: full Budget-domain currency calculations (plan section 16 - Budget's own currency, FX-converted linked Transactions/Accounts using historical rates) - currently Budgets still sum raw `transaction.amount`/read `Budget.goalAmount` without conversion, an accepted, documented limitation, not a bug to silently fix later.
  - The orphaned transfer leg (`_id: 6a890eed...37d3`, real `transferGroupId` now pointing to a deleted sibling) is real data hygiene debt, not urgent - it displays and calculates correctly now (excluded from income/expense), it's just missing its counterpart leg.
  - Revolut (USD) Account and its one transaction are real, ongoing data the user intends to keep using - not test data to clean up.
  - Standing rule unchanged: explicit permission required before any real MongoDB document write. This session's real-data actions (the full `--confirm` migration, the manual-override edit test) were both explicitly approved beforehand.
  - `claude/multicurrency` is not yet merged anywhere.

---

### 📅 Entry #30: 2026-09-07/08 (local time) — App-wide dark "violet glass" theme polish pass (PR #15, merged to main)

- **AI Assistant**: Claude (Sonnet 5)
- **User Request**: A long, iterative live-UI-review session (voice-dictated feedback in Spanish, often with hand-annotated screenshots) rolling the existing "violet glass" 3D-glassmorphism aesthetic out consistently across the whole app - Dashboard, Wallet, Budgets, Accounts, Categories, Projections, History. Standing instructions for the session: enumerate every feedback item before touching code; the Top-6 drill-down modal (`ModalContentTopMonthItem`, `gf-glass-violet`) is the reference translucency for modals; the category Treemap's hover tooltip is the reference for every chart tooltip app-wide; verify every fix live in the browser (not just by reading the diff) before reporting it done.
- **Phase**: Not part of the multi-currency plan - a separate, purely visual/UX theming pass across already-shipped features. No schema, route, or data changes.
- **Actions Taken** (representative, not exhaustive - this ran many iterative rounds):
  1. **Z-index whack-a-mole across every portal-based popup system**: MUI DatePicker (`.MuiPickersPopper-root`/`.MuiDialog-root`), antd DatePicker (`.ant-picker-dropdown`), antd Popover/Tooltip (`.ant-popover`/`.ant-tooltip`) all defaulted to a low z-index that lost to modal backdrops, rendering genuinely-open-but-invisible-behind-the-modal. Fixed with global `:root` overrides in `globals.css` (`z-index: 30000 !important` on each). A related but distinct bug: antd's *imperative* `Modal.confirm()` API (used for delete-movement confirmations) renders outside the app's own React tree/ConfigProvider entirely, so no CSS override could reach it - fixed by converting each usage to a controlled, declarative `<Modal open={state} className="gf-antd-modal-glass" zIndex={20000} />` instead.
  2. **New/extended CSS glass utilities** in `globals.css`: `.gf-glass-tile` (raised 3D shell for category-colored circles/tiles that can't use a translucent background since there's nothing to blur behind an opaque brand color) went through two iterations - a box-shadow-only bevel that looked wrong and didn't cover a tile's own outer border ring, replaced with a `::before` pseudo-element carrying a translucent diagonal gradient (same 160deg light-to-dark direction as the navbar's `+` button, `.gf-glass-fab`), extended past the fill into a caller-drawn border via a `--gf-tile-border` CSS variable (SubCategoryList's father-category-colored ring). `.gf-glass-info` (new, blue) added alongside the existing warning/success variants. `.gf-antd-modal-glass` fixes antd's `.ant-modal-header` painting its own flat solid fill independent of `.ant-modal-content`'s gradient.
  3. **Chart tooltip/label dark-theme pass**: `@ant-design/plots` Column charts default to a light theme (`theme: "classicDark"` added at the base `ColumnChartAntComparative.jsx` level, cascades through every caller's own tooltip override). Rebuilt three separate custom `interaction.tooltip.render()` implementations (Bills/Incomes Comparative tab, Compare-periods tab, Budgets-comparative per-budget chart) to match the Treemap's `gf-glass-chip` dark-glass style with properly aligned color-dot/label/value rows. On-bar value labels illegible against light-colored bars fixed with G2's built-in `contrastReverse` label transform (not a hand-rolled stroke halo, which was tried first and looked bad) - G2 doesn't support a background-chip label the way the Treemap's own tile labels do. **Root-caused, after three failed "fixed it" attempts, a persistent tooltip backdrop the user kept seeing**: turned out to be two separate, unrelated causes - `@ant-design/plots`' Column chart has its own default `interaction: { elementHighlight: { background: true } }` (undocumented in this app's code, found by reading the library's own `getDefaultOptions()` source), entirely separate from the tooltip's own `crosshairs` option (which also needed disabling); and G2's own tooltip DOM container (`div.g2-tooltip`) wraps every custom `render()` output with its own opaque `rgb(31,31,31)` background, padding, and a barely-rounded 4px border-radius, which was still fully visible around the rounded glass content - found only by inspecting `getComputedStyle()` on the live element via the browser tools, not by guessing. Fixed with an explicit `crosshairs: false` + `elementHighlight: false` in every chart's interaction config, plus a global `.g2-tooltip { background/padding/box-shadow/border-radius: 0 }` override.
  4. **Real functional bugs found and fixed along the way** (not stylistic): `ResumeTabsTrans.jsx` accepted an `rttTrans` prop (meant to scope it to one Account on the Accounts page) but never actually read it, always pulling the full wallet's Redux transaction list instead - so switching the selected Account via AccountClient's carousel silently left "Transactions Resume" showing stale, wrong data. `TransDetailsGrandContainer.jsx` called the newly-added `DisplayerCategoryTreemap` with mismatched prop names (`dctIncomes`/`dctBills` instead of `dccpIncomes`/`dccoBills`), crashing the entire Dashboard page with `Cannot read properties of undefined (reading 'length')` - found only because a routine live-verification pass hit the crash, not because it was reported.
  5. **Ambient background seam bug**: the dashboard shell's ambient mesh background (`.walllet-bg`, 5 radial-gradient blobs) was tiling every `100vh` down the page via `background-repeat: repeat`, but the blob pattern isn't seamlessly tileable (the value at a tile's bottom edge doesn't match its top edge), producing a hard horizontal line at every scroll-height multiple, confirmed reproducible on Dashboard and Categories both. Root-caused by hiding every other layer on the page via injected CSS and screenshotting the isolated background alone - not fixed by guesswork. Fixed by restoring `background-attachment: fixed` (which the surrounding code comments already described as the intended, previously-tested design - the tiled-repeat version was an undocumented regression from that).
  6. Dozens of smaller, itemized fixes across the session: Edit/Create Account modal's Currency/Account Type selects rendering with a raw white browser-default background in dark mode; "Remove account"/"Delete budget"/"Delete project" converted from underlined text links to proper `gf-glass-button-danger` buttons; a CSS-specificity bug (`.form-trans-edit input` beating `.gf-glass-button` on `<input type="submit">`, fixed by converting to `<button type="submit">`); a budget progress-bar percentage badge's centering formula (`pct% - pct*0.4px`, only exactly centered at pct=50) replaced with the correct `clamp(0px, calc(pct% - halfWidth), calc(100% - width))` form, then found to still differ between the row and modal renderings of the same badge and unified; native number-input spin buttons hidden app-wide (can't be recolored via CSS to match a custom theme, the standard fix); Add Transaction modal's active tab given a raised glass pill instead of a barely-different solid fill; Projections' month-detail chart missing a dark theme; Account movement details section missing its own independent date-range filter (added, matching Transactions Resume's existing one).
- **Files Created / Modified**: 126 files in the merged PR - see `git show --stat` on commit `8ef64e4` / PR #15 for the full list. Highest-touch: `src/app/globals.css` (the great majority of new/changed CSS utilities), `src/components/multiUsedComp/Budgets/BudgetDetailModal.jsx`, `AccountClient.jsx`, `ResumeTabsTrans.jsx`, `TransDetailsGrandContainer.jsx`, `EditAccountModal.jsx`, `Movements.jsx`, the three chart-tooltip config files (`ColumnChartAntComparative.jsx`, `propsColTabsToggler.js`, `propsForBudgetMonthlyChart.js`), `EditCategoryModal.jsx`, `SubCategoryList.jsx`/`CategoryList.jsx`, `CategoriesClient.jsx`, `HistoricalBaselinePanel.jsx`/`IncomeSourcesPanel.jsx`.
  - This entry: `.mds/AI_COORDINATION_LOG.md`.
  - Database: none - purely a front-end/CSS/component pass, no schema or data changes.
- **Test status at end of session**: 249/249 vitest passing throughout every round of changes (full project suite, unrelated to this session's UI work specifically, used as a regression safety net), zero new ESLint errors on any touched file. Committed as `8ef64e4` on `claude/multicurrency`, opened as PR #15, merged to `main` with a regular merge commit (matching the convention of every prior PR from this branch), then `claude/multicurrency` deleted (both locally and on `origin`) since it was fully merged with nothing left unmerged.
- **Next Steps / Hand-Off Notes**:
  - This branch (`claude/multicurrency`) is now closed/deleted. New work continues on a fresh branch, `agents-ai`, cut from the now-current `main`.
  - Next feature, not yet started: an AI-facing "monthly summary" format - assembling the Dashboard's existing Wallet Analyzer insights, Top-6/Top-12 lists, and account data into a structured payload the user can hand to an LLM (ChatGPT/Gemini/Claude, via the app's existing MCP connectors, or by asking Claude directly) for a monthly analysis, with the app itself already providing organized data and some pre-computed conclusions - the open design question, still being scoped, is what real value an LLM template adds on top of data the app already summarizes for the user.
  - No known regressions or deferred bugs from this session - everything reported by the user was fixed and verified live before being reported done, except where the user explicitly said not to touch something (UnbudgetedSpending's category-row tint) or explicitly confirmed something was already correct.
